...
Hi there! I am the finq web environment, and will help you navigate the internet.
...
"""
    ...
    @override
    def which(self, text: str) -> Literal["observe", "step", "scrape", "rules"]:
        url = self.search_pattern(text, BasefinqParser.observe_tag)
        action = self.search_pattern(text, BasefinqParser.step_tag)
        scrape = f"<{BasefinqParser.done_tag}/>" in text
    ...
    def is_done(self, text: str) -> bool:
        return (
            f"<{BasefinqParser.done_tag}/>" in text or self.search_pattern(text, BasefinqParser.done_tag) is not None
        )
    ...
    def get_done_answer(self, text: str) -> str | None:
        ...
        return self.search_pattern(text, BasefinqParser.done_tag)
