...
from finq.actions.base import ActionParameter, PossibleAction
...
def parse_action_parameters(action: str) -> list[ActionParameter]:
    """
Parse action parameters from a string into ActionParameter objects.
...
"""
    ...
    def parse_values(values_str: str) -> list[str]:
        """
        Split a parameter string into individual parts while handling nested brackets.

        Args:
            parameters_str (str): The parameter string to be split.

        Returns:
            list[str]: A list of individual parameter parts.
        """
...
def parse_markdown_action_list(
    ...
    for line in markdown_content.split("\n"):
        try:
            ...
            # Check if the line contains any disabled action and skip it
            if any(disabled in line.lower() for disabled in DISABLED_ACTIONS):
