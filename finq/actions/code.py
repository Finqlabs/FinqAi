from finq.actions.base import ActionParameterValue, ExecutableAction
from finq.browser.context import Context
from finq.browser.node_type import HtmlSelector, finqNode
...
def get_action_from_node(node: finqNode) -> str:
    if node.id is None:
...
def compute_playwright_code(
    ...
    node = action.node
    if node is None:
        raise ValueError(f"Target finq node for action {action.id} must be provided")
    if node.attributes_post is None:
