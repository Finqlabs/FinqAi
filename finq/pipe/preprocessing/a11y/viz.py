...
from finq.browser.node_type import A11yNode
...
def a11tree_to_tree_string(
    ...
    finq_id_str = "" if "id" not in node else f" ({node['id']})"
    node_text = f"[{node['role']}{group_role_str}] '{node.get('name', '')}' {finq_id_str}"
...
def a11tree_to_markdown(node: A11yNode, heading_level: int = 1) -> str:
    ...
    finq_id_str = f" ({node['id']})" if "id" in node else ""
    ...
    if role == "link":
        ...
        markdown += f"[{link_text}]({link_href}){finq_id_str}\n"

    elif role == "button":
        markdown += f"Button:{name}{finq_id_str}\n"
        ...
        if not last_line.startswith("#") and not markdown.endswith("\n\n"):
            markdown += "\n"
        markdown += f"{'#' * heading_level} {name}{finq_id_str}\n"
        for child in node.get("children", []):
            markdown += a11tree_to_markdown(child, heading_level + 1)

    else:
        if name:
            markdown += f"{name}{finq_id_str}\n"
