...
from finq.browser.node_type import A11yNode, A11yTree
from finq.pipe.preprocessing.a11y.id_generation import (
    generate_sequential_ids,
    sync_ids_between_trees,
    sync_image_ids_between_trees,
)
from finq.pipe.preprocessing.a11y.pruning import (
...
# from finq.pipe.preprocessing.a11y.text import prune_text_nodes
from finq.pipe.preprocessing.a11y.traversal import (
    find_all_paths_by_role_and_name,
    find_node_path_by_id,
    list_interactive_nodes,
)
from finq.pipe.preprocessing.a11y.validation import (
    check_interactions_consistency_accross_ax_trees,
)
from finq.pipe.preprocessing.a11y.viz import visualize_a11y_tree
...
class ProcessedA11yTree:
    ...
    def find_node_path_by_id(self, id: str, type: str = "processed") -> list[A11yNode] | None:
        match type:
            case "processed":
                return find_node_path_by_id(node=self.processed_tree, finq_id=id)
            case "simple":
                return find_node_path_by_id(node=self.simple_tree, finq_id=id)
            case "raw":
                return find_node_path_by_id(node=self.raw_tree, finq_id=id)
            case _:
                raise ValueError(f"Unknown type {type}")
