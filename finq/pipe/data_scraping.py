...
from finq.browser.driver import BrowserDriver
from finq.browser.node_type import finqNode
from finq.data.space import DataSpace, ImageCategory, ImageData
...
from finq.sdk.types import ScrapeRequest, ScrapeRequestDict
from finq.utils.image import construct_image_url
...
async def resolve_image_conflict(page: Page, node: finqNode, node_id: str) -> Locator | None:
    ...
    async def forward(
        ...
        **param: Unpack[ScrapeRequestDict],
        ...
        if len(self.token_encoder.encode(document)) > self.max_tokens:
            ...
            simple_node = finqNode.from_a11y_node(tree.simple_tree, path=context.snapshot.metadata.url)
