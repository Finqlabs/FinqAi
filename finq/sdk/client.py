...
from finq.data.space import DataSpace
from finq.sdk.types import (
    ObserveRequest,
    ObserveRequestDict,
    ObserveResponse,
    ScrapeRequest,
...
class finqClient:
    """
    Client for the finq API.

    Note: this client is only able to handle one session at a time.
    If you need to handle multiple sessions, you need to create a new client for each session.
    """

    DEFAULT_SERVER_URL: ClassVar[str] = "https://api.finq.cc"
    ...
    def __init__(
        ...
        self.token = api_key or os.getenv("finq_API_KEY")
        if self.token is None:
            raise ValueError("finq_API_KEY needs to be provided")
    ...
    def start(self, **data: Unpack[SessionRequestDict]) -> SessionResponse:
        if self._base_session_request is not None and self._base_session_request.session_id is not None:
            logger.warning("One finq session already exists. Closing it before starting a new one...")
            _ = self.close()
