...
class finqNode:
    ...
    children: list["finqNode"] = field(default_factory=list)
    attributes_pre: NodeAttributesPre = field(default_factory=NodeAttributesPre.empty)
    attributes_post: finqAttributesPost | None = None
    ...
    @staticmethod
    def from_a11y_node(node: A11yNode, path: str = "") -> "finqNode":
        node_path = ":".join([path, node["role"], node["name"]])
        children = [finqNode.from_a11y_node(child, node_path) for child in node.get("children", [])]
        return finqNode(
            id=node.get("id"),
    ...
    def find(self, id: str) -> "finqNode | None":
        if self.id == id:
            return self
        for child in self.children:
            found = child.find(id)
    ...
    def flatten(self, only_interaction: bool = False) -> list["finqNode"]:
        base: list["finqNode"] = [] if only_interaction and not self.is_interaction() else [self]
    ...
    def image_nodes(self) -> list["finqNode"]:
        return [node for node in self.flatten() if node.is_image()]

    def subtree_filter(self, ft: Callable[["finqNode"], bool]) -> "finqNode | None":
        def inner(node: finqNode) -> finqNode | None:
            children = node.children
            if not ft(node):
                return None

            filtered_children: list[finqNode] = []
    ...
    def subtree_without(self, roles: set[str]) -> "finqNode":

        def only_roles(node: finqNode) -> bool:
...
class InteractionNode(finqNode):
    id: str  # type: ignore
